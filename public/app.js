...
async function completeService(e,id){e.preventDefault();const form=e.currentTarget,fd=new FormData(form),btn=form.querySelector('button[type=submit]');fd.set('services',JSON.stringify($$('input[name=services]:checked',form).map(i=>i.value)));try{btn.disabled=true;btn.textContent='Finalizando e enviando e-mail...';const result=await api(`/api/maintenances/${id}/complete`,{method:'POST',body:fd});await refreshActiveService();if(result.email_notification?.sent)toast(`Serviço finalizado e notificação enviada para ${result.email_notification.recipient_count} destinatário(s).`);else toast(result.email_notification?.message||'Serviço finalizado, mas a notificação não foi enviada.',true);go('/dashboard');}catch(err){toast(err.message,true);}finally{btn.disabled=false;btn.textContent='Finalizar Serviço';}}
