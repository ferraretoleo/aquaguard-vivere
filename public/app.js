...
function chartSvg(rows) {
  ...
  const ph=recent.map(r=>r.ph), chlorine=recent.map(r=>r.chlorine), alk=recent.map(r=>Number(r.alkalinity)/10);
  return `<svg class="chart-svg" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" aria-label="Evolução dos parâmetros químicos">
    ...
    <polyline class="chart-line" stroke="#2563eb" points="${linePoints(ph,w,h)}"/><polyline class="chart-line" stroke="#16a34a" points="${linePoints(chlorine,w,h)}"/><polyline class="chart-line" stroke="#d97706" points="${linePoints(alk,w,h)}"/>
  </svg>`;
}

function isIdeal(type,value){ const n=Number(value); return type==='ph'?(n>=7.2&&n<=7.6):type==='chlorine'?(n>=1&&n<=3):(n>=80&&n<=120); }
...
function historyCard(m) {
  return `<article class="history-card"><div><h3>${esc(m.pool_name)} - ${esc(m.location_name)}</h3><p>${esc(m.executor)}</p><p>${brDate(m.started_at)}</p></div><div><div class="chips"><span class="chip ${isIdeal('ph',m.ph)?'good':''}">pH: ${esc(m.ph)}</span><span class="chip ${isIdeal('chlorine',m.chlorine)?'good':''}">Cloro: ${esc(m.chlorine)}</span><span class="chip ${isIdeal('alk',m.alkalinity)?'good':''}">Alc: ${esc(m.alkalinity)}</span></div><p>${(m.services||[]).length} serviço(s) realizado(s)</p></div><button class="history-open" data-id="${m.id}" title="Ver detalhes">⌕</button></article>`;
}
